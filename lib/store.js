...
export const addTask = async (task) => {
    ...
    if (error) {
        console.error('addTask error details:', error.message || error);
        return null;
    }
    return data;
};
...
// ── Adjust task date by ±N days ───────────────────────────────
export const adjustTaskDate = async (id, deltaDays) => {
    const { data: task, error: fetchErr } = await supabase
        .from('tasks').select('date').eq('id', id).single();
    if (fetchErr) { console.error('adjustTaskDate fetch error:', fetchErr); return null; }

    const newDate = format(addDays(new Date(task.date + 'T00:00:00'), deltaDays), 'yyyy-MM-dd');
    const { data, error } = await supabase
        .from('tasks')
        .update({ date: newDate, notified: false })
        .eq('id', id)
        .select()
        .single();
    if (error) { console.error('adjustTaskDate error:', error); return null; }
    return data;
};

// ── Reschedule single task — smart: today if time not yet passed, else tomorrow ──
export const rescheduleToNextDay = async (id) => {
    // First fetch the task so we know its time
    const { data: task, error: fetchErr } = await supabase
        .from('tasks')
        .select('*')
        .eq('id', id)
        .single();
    if (fetchErr) { console.error('rescheduleToNextDay fetch error:', fetchErr); return null; }

    // Work out the target date
    const now = new Date();
    ...
    if (task?.time) {
        ...
        // Build a Date for the task's time on today
        const taskTimeToday = new Date(now);
        taskTimeToday.setHours(h24, minute, 0, 0);
    ...
    const { data, error } = await supabase
        .from('tasks')
        .update({ date: targetDate, notified: false, completed: false })
        .eq('id', id)
        .select()
        .single();
...
// ── Reschedule specific tasks by IDs ─────────────────────────
export const rescheduleTasksByIds = async (ids) => {
    if (!ids || ids.length === 0) return await getTasks();
    ...
    const { error: updateError } = await supabase
        ...
        .in('id', ids);
    if (updateError) { console.error('rescheduleTasksByIds update error:', updateError); }
